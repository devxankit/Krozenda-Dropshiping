        ...
        return (
          <li key={`${event.label}-${index}`} className="flex gap-3">
            ...
            <div className={`min-w-0 flex-1 ${isLast ? 'pb-0' : 'pb-4'}`}>
              <p
                className={`text-xs font-semibold ${event.done ? 'text-slate-900' : 'text-ink-faint'}`}
              >
                {event.label}
              </p>
              {event.at && (
              ...
              {event.reason && (
                <p className="mt-1 text-2xs leading-snug text-ink-subtle">{event.reason}</p>
              )}
            </div>
          </li>
        )
    ...
    <span className="flex items-center justify-end gap-1">
      {items.map((item) => (
        <button
          ...
          className="rounded-md p-1.5 text-ink-faint transition-colors hover:bg-surface-muted hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
        >
