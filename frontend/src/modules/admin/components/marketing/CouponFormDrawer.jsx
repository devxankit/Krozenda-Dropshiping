...
import { Badge, Checkbox, Input, Select, SegmentedControl } from '../../../../components/ui'
...
const APPLICABLE_TO_OPTIONS = [
  { value: 'ALL', label: 'Entire storefront' },
  { value: 'PRODUCTS', label: 'Specific products' },
  { value: 'CATEGORIES', label: 'Specific categories' },
  { value: 'VENDORS', label: 'Specific sellers' },
]
...
export function CouponFormDrawer({ isOpen, onClose, coupon, writer }) {
  ...
  useEffect(() => {
    if (!isOpen) return
    if (form.applicableTo === 'PRODUCTS' && products.length === 0) {
      api.get('/admin/catalog/products').then(({ data }) => setProducts(data.data.items)).catch(() => {})
    }
    if (form.applicableTo === 'CATEGORIES' && categories.length === 0) {
      api.get('/admin/catalog/categories').then(({ data }) => setCategories(data.data.items)).catch(() => {})
    }
    if (form.applicableTo === 'VENDORS' && vendors.length === 0) {
      api.get('/admin/vendors').then(({ data }) => setVendors(data.data.items)).catch(() => {})
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, form.applicableTo])
  ...
  function handleSubmit(event) {
    ...
    if (form.applicableTo === 'PRODUCTS' && form.productIds.length === 0) return setIssue('Select at least one product')
    if (form.applicableTo === 'CATEGORIES' && form.categoryIds.length === 0) return setIssue('Select at least one category')
    if (form.applicableTo === 'VENDORS' && form.vendorIds.length === 0) return setIssue('Select at least one seller')
    setIssue(null)
    ...
    const payload = {
      ...
      productIds: form.applicableTo === 'PRODUCTS' ? form.productIds : [],
      categoryIds: form.applicableTo === 'CATEGORIES' ? form.categoryIds : [],
  ...
  return (
    ...
    >
      ...
      <Select
        id="coupon-applicable-to"
        label="Applies To"
        options={APPLICABLE_TO_OPTIONS}
        value={form.applicableTo}
        onChange={(event) => setForm((c) => ({ ...c, applicableTo: event.target.value }))}
      />

      {form.applicableTo === 'PRODUCTS' && (
        <div className="max-h-48 overflow-y-auto rounded-xl border border-slate-200 p-2">
          {products.length === 0 && <p className="p-2 text-2xs text-slate-400">Loading products…</p>}
          {products.map((p) => (
      ...
      {form.applicableTo === 'CATEGORIES' && (
        <div className="max-h-48 overflow-y-auto rounded-xl border border-slate-200 p-2">
          {categories.length === 0 && <p className="p-2 text-2xs text-slate-400">Loading categories…</p>}
          {categories.map((c) => (
      ...
      {form.applicableTo === 'VENDORS' && (
        <div className="max-h-48 overflow-y-auto rounded-xl border border-slate-200 p-2">
          {vendors.length === 0 && <p className="p-2 text-2xs text-slate-400">Loading sellers…</p>}
          {vendors.map((v) => (
            <label key={v.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs hover:bg-slate-50">
              <Checkbox checked={form.vendorIds.includes(v.id)} onChange={() => toggleId('vendorIds', v.id)} />
              <span className="truncate">{v.business?.businessName || v.name}</span>
            </label>
          ))}
        </div>
      )}
      ...
      <Select
        ...
        onChange={(event) => setForm((c) => ({ ...c, customerEligibility: event.target.value }))}
      />
