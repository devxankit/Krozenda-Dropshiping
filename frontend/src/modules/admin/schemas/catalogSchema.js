...
export const productWriteSchema = z
  .object({
    ...
    price: z.number().min(0, 'Enter a valid selling price'),
    mrp: z.number().min(0, 'MRP cannot be negative').nullable().optional(),
    costPrice: z.number().min(0, 'Cost price cannot be negative').nullable().optional(),
    salePrice: z.number().min(0).nullable().optional(),
    discountPercent: z.number().min(0).max(100).optional(),
    stock: z.number().int().min(0, 'Stock cannot be negative'),
    lowStockThreshold: z.number().int().min(0, 'Threshold cannot be negative').nullable().optional(),
    weight: z.number({ required_error: 'Weight (kg) is required', invalid_type_error: 'Weight must be a valid number' }).positive('Weight must be greater than 0 kg'),
